...
async function setupAgent() {
  try {
    ...
    registrationToken = registrationResponse.data.data.registrationToken;
...
async function bootstrapChainParticipation(notification) {
  try {
    ...
    const registerResponse = await chainsApi.post('/register', {
    ...
    const validatorResponse = await chainsApi.post(`/${chainInfo.humanReadableId}/validators`, {
    ...
    const subscriptionResponse = await chainsApi.post(`/${chainInfo.humanReadableId}/subscribe`, {
      agentId: agentId,
